<style>
	#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM.modal .modal-dialog .modal-content .modal-body {
		padding-top: 15px;
		/* padding-bottom: 5px; */
	}

	#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM #{{ $sectionId }}_GO_BACK_BTN {
		display: none;
	}

	#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM .modal-body {
		min-height: 50px;
	}
	
	@media ( max-width : 575px) {
		#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM .modal-footer {
...
<div class="modal fade" id="{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM" tabindex="-1"
	role="dialog" aria-labelledby="LIST-DANH-MUC-SAN-PHAM-LABEL"
	aria-hidden="true" 
	...
	<div class="modal-dialog modal-dialog-centered modal-md modal-dialog-scrollable" role="document"
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h5 class="modal-title" id="LIST-DANH-MUC-SAN-PHAM-LABEL">Chọn danh mục</h5>

				<button type="button" class="close btn rounded-circle" data-bs-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">×</span>
				</button>
			</div>
			<div class="modal-body disable-events" id="modal-body-list-card-dm-san-pham">
				<style>
				...
				<section id="{{ $sectionId }}">
					<!-- START block template card sản phẩm -->
					<div tree-level="{-{TREE_LEVEL}-}" id="{{ $sectionId }}_danhMucSanPhamTemplateRow" class="d-none">
								...
								<div class="card mb-3 card-body">
									<div class="row align-items-center">
										<div class="col-auto col-img">
											<{-{TAG_IMG}-} src="{-{SRC_IMG}-}" class="width-90 rounded-3" alt="">
										</div>
					...
					<!-- END block template card sản phẩm -->

					<div id="{{ $sectionId }}_danhMucSanPhamRow" class="row list-card">
					</div>
						...
						<div class="col-xl-12 col-card">
							<a href="javascript:void(0);" class="text-reset text-decoration-none" action-type="item-card" data-label="Danh mục con 1" data-id="1" data-parent-id="null">
								<div class="card mb-3 card-body">
									<div class="row align-items-center">
										<div class="col-auto col-img">
											<img src="https://png.pngtree.com/png-vector/20190901/ourlarge/pngtree-real-estate-icon-design-vector-png-image_1711820.jpg" class="width-50 rounded-3" alt="">
										</div>
										<div class="col col-description">
											<div class="overflow-hidden flex-nowrap">
												<h6 class="mb-1" style="padding-top: 5px; line-height: 1.2rem;">
...
var {{ $sectionId }}_treeLevelListSanPham = 0;

$(document).ready(function(){
	/* Xử lý khi open popup */
	{{ $sectionId }}_handleOpenPopupSanPham = function() {
		// Reset tree level
		{{ $sectionId }}_treeLevelListSanPham = 0;
		...
		$('#{{ $sectionId }}_danhMucSanPhamRow > [id^="{{ $sectionId }}_TREE_LEVEL_"]').each(function(index) {
	...
	$('#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM').on('show.bs.modal', function (e) {
		// Xử lý sự kiện khi modal bắt đầu mở. Đang chuyển cảnh...
		console.log('Modal đang mở!');
		$('#modal-body-list-card-dm-san-pham').removeClass('disable-events').addClass('disable-events');
	});

	/* Xử lý sự kiện khi modal ĐÃ MỞ */
	$('#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM').on('shown.bs.modal', function (e) {
		// Xử lý sự kiện khi modal đã mở. Hoàn tất chuyển cảnh
		console.log('Modal đã mở!');
		$('#modal-body-list-card-dm-san-pham').removeClass('disable-events');
	});
	
	/* Xử lý sự kiện khi modal ĐANG ĐÓNG */
	$('#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM').on('hide.bs.modal', function (e) {
		// Xử lý sự kiện khi modal đang đóng. Đang chuyển cảnh...
		console.log('Modal đang đóng!');
	});

	/* Xử lý sự kiện sau khi MODAL ĐÃ ĐÓNG */
	$('#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM').on('hidden.bs.modal', function (e) {
	...
	$('#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM').on('hidePrevented.bs.modal', function (e) {
		// Ngăn modal focus lại khi click vào backdrop
		e.preventDefault();
	});
	
	/* Get danh sách danh mục sản phẩm */
	function {{ $sectionId }}_getListDanhMucSanPham(parentId) {
		$.ajax({
			type : "GET",
			url : '{{ url("/api/categoryp/list") }}',
			...
			success : function(data, textStatus, request) {
				...
				$('#{{ $sectionId }}_TREE_LEVEL_' + {{ $sectionId }}_treeLevelListSanPham).remove();

				// Xử lý thêm một block section div tree level
				let newSectionTreeLevel = '<div tree-level="' + {{ $sectionId }}_treeLevelListSanPham + '" id="{{ $sectionId }}_TREE_LEVEL_' + {{ $sectionId }}_treeLevelListSanPham + '"></div>';
				$('#{{ $sectionId }}_danhMucSanPhamRow').append(newSectionTreeLevel);

				let templateRow = $('#{{ $sectionId }}_danhMucSanPhamTemplateRow').html();

				let arrResult = [];
				var result = data.DATAS.CATEGORY_P.DATA;
				if (result && result.length > 0) {
					for (let i = 0; i < result.length; i++) {
						let idVal = result[i]['ID'] || '';
						let nameVal = result[i]['TEN_DANH_MUC_SAN_PHAM'] || '';
						...
						// Append after div
						$('#{{ $sectionId }}_TREE_LEVEL_' + {{ $sectionId }}_treeLevelListSanPham).append(newRow);
					}
				}
			...
			complete : function() {
				...
				// Scroll on top
				$('#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM .modal-body').scrollTop(0);
	...
	{{ $sectionId }}_getListDanhMucSanPham();
	...
	function {{ $sectionId }}_clickItemCardDanhMuc(currSectionId, currSectionLevel, dataLabel, dataId, dataParentId, countChildren, pathView) {
		...
		if (countChildren == 0) {
			// Đóng modal popup
			console.log($('#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM').hasClass('show'));
			if ($('#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM').hasClass('show')) {
				$('#{{ $sectionId }}_MODAL-LIST-DANH-MUC-SAN-PHAM').modal('hide'); // Chỉ ẩn modal nếu nó đang mở
			}

			{{ $sectionId }}_callBack_danhMucSanPham(dataLabel, dataId, pathView);
			return;
		}

		// Ẩn current section để mở section child
		$('#{{ $sectionId }}_TREE_LEVEL_' + {{ $sectionId }}_treeLevelListSanPham).hide();
		// Xóa tất cả các active thẻ card item .card.mb-3.card-body
		$('#' + currSectionId + ' .card.mb-3.card-body').removeClass('active');
		

		// Hiển thị section child
		{{ $sectionId }}_treeLevelListSanPham++;
		{{ $sectionId }}_getListDanhMucSanPham(dataId);

	}

	/* Xử lý click go back */
	$('#{{ $sectionId }}_GO_BACK_BTN').on('click', function(e) {
		if ({{ $sectionId }}_treeLevelListSanPham === 0) return;

		// Ẩn section hiện tại
		$treeLevelCurr = $('#{{ $sectionId }}_TREE_LEVEL_' + {{ $sectionId }}_treeLevelListSanPham);
		$treeLevelCurr.hide();
			
		// Hiển thị section parent trước đó
		{{ $sectionId }}_treeLevelListSanPham--;
		$treeLevelParent = $('#{{ $sectionId }}_TREE_LEVEL_' + {{ $sectionId }}_treeLevelListSanPham);
	...
	{{ $sectionId }}_handleShowHideGoBackBtn = function() {
		if ({{ $sectionId }}_treeLevelListSanPham > 0) {
