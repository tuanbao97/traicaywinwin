...
<style>
	@media (min-width: 1500px) {
		#CHI_TIET_DANH_MUC_SAN_PHAM .section.col-lg-4-custom {
			flex: 0 0 auto;
			width: 350px;
		}
		#CHI_TIET_DANH_MUC_SAN_PHAM .section.col-lg-8-custom {
			flex: 0 0 auto;
...
<div class="row section-main" id="SECTION_MAIN">
	...
	<div class="col-lg-12 grid-margin stretch-card" id="CHI_TIET_DANH_MUC_SAN_PHAM">
		<div class="card">
			<div class="card-body">
				<div class="row justify-content-between">
					<div class="col-md-12 col-sm-12">
						<h4 class="card-title">
							CHI TIẾT <span class="one-line">DANH MỤC SẢN PHẨM</span>
						</h4>
					</div>
				</div>
					...
					<div class="row">
						...
						<div class="section col-lg-8-custom col-md-12">
							<div class="form-group col-md-12 col-sm-12">
    							<label for="NAME">Tên danh mục sản phẩm<code>*</code></label> 
    							<input type="text"
    								class="form-control" id="EDIT_TEN_DANH_MUC_SAN_PHAM" placeholder="">
    							<span class="error-message" id="MSG_EDIT_TEN_DANH_MUC_SAN_PHAM"></span>
    						</div>
    						<div class="form-group col-md-12 col-sm-12">
							...
							<div class="form-group col-md-12 col-sm-12">
    							<label for="NAME">Đường dẫn file giao diện</label> 
    							<input type="text"
    								class="form-control" id="EDIT_PATH_VIEW" placeholder="">
    							<span class="error-message" id="MSG_EDIT_PATH_VIEW"></span>
    						</div>
...
$(document).ready(function () {	
	@if (isset($categoryPId))
		/* Xử lý get chi tiết category product */
		var categoryPId = '{{ isset($categoryPId) ? $categoryPId : null }}';
		...
		$.ajax({
			type : "GET",
			url: '{{ url("/api/categoryp/detail") }}' + "/" + categoryPId, 
			contentType : "application/json",
			showLoading: true,
			traditional: true,
			data : data,
			success : function(data, textStatus, request) {
				data = data.DATAS.CATEGORY_P;
				...
				// Set selected value
				getListDanhMucSanPhamTree(data['PARENT_ID']);
	...
	$('button[name="BTN_SAVE"]').on('click', function() {
    	...
    	$.ajax({
    		...
    		success: function(data, textStatus, request) {
    			...
    			$('#EDIT_ID').val(data.DATAS.CategoryP.ID);

    			// Reload danh sách catetory parent
    			getListDanhMucSanPhamTree(data.DATAS.CategoryP.PARENT_ID);

    			// Cập nhật id vừa save
    			updUrlCategoryId(data.DATAS.CategoryP.ID);
    		}, 
    		error: function(request, textStatus, errorThrown) {
				...
				try {
					...
					// Scroll đến msg lỗi
					scrollMsgInSection($('#CHI_TIET_DANH_MUC_SAN_PHAM'));
				}
    		},
    		complete: function() {
    		}
 	...
 	function updUrlCategoryId(categoryPId) {
 	 	let url = '{{ url("/admin/danh-muc-san-pham/chi-tiet") }}'
 	 	if (categoryPId !== null) {
 	 		url= url + "/" + categoryPId;
 	 	}
	...
	function setInputData() {
    	...
    	var data = {
			...
			PARENT_ID : $("#EDIT_PARENT_ID").val() || null,
   			TEN_DANH_MUC_SAN_PHAM : $('#EDIT_TEN_DANH_MUC_SAN_PHAM').val() || null,
			...
			PATH_VIEW : $('#EDIT_PATH_VIEW').val() || null,
    ...
    $('button[name="BTN_GO_BACK"]').on('click', function() {
    	window.location = '{{ url('/admin/danh-muc-san-pham') }}';
	});
		...
		$("#EDIT_PARENT_ID").prepend('<option selected></option>').select2ToTree(
			{
				...
				treeData: {
					...
					labelFld: "TEN_DANH_MUC_SAN_PHAM",
	...
	function getListDanhMucSanPhamTree(selectedVal) {
		$.ajax({
			type : "GET",
			...
			success : function(data, textStatus, request) {
				...
				let arrResult = [];
				var result = data.DATAS.CATEGORY_P;
	...
	$('button[name="BTN_DELETE"]').on('click', function() {
		let categoryPId = $('#EDIT_ID').val() != null ? $('#EDIT_ID').val() : '';
