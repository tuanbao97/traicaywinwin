					...
					<div class="card-body">
						<div class="row">
							...
							<div class="col-md-3 col-sm-12">
								<div class="form-group">
									<label for="exampleInputActive">Loại video</label>
									<select id="SEARCH_LOAI_VIDEO"
			...
			<div class="card-body">
				<div class="row justify-content-between">
					<div class="col-md-8 col-sm-12">
						<h4 class="card-title">
							DANH SÁCH <span class="one-line">VIDEO</span>
						</h4>
					</div>
					<div class="col-md-4 col-sm-12 float-right text-align-right">
						<a href="{{ url('/admin/video/chi-tiet') }}">
							<button type="button"
					...
					<table id="tableVideo"
						...
						<tfoot>
                            <tr>
                                ...
                                <th class="header-footer">Tiêu đề video</th>
								<th class="header-footer">Tóm tắt</th>
								<th class="header-footer">Loại video</th>
...
$(document).ready(function(){
	...
	dataTableVideo = new DataTable('#tableVideo', {
		...
		, ajax: {
			...
			, data : function(dataInput) { // Đây là function riêng của Datatables
				...
				dataInput.LOAI_VIDEO = $('#SEARCH_LOAI_VIDEO').val();
			...
			, dataSrc: function (json) {
				...
				if (json.STATUS === true) {
					// Tính toán Paging
					if (json.DATAS.VIDEO.DATA.length > 0) {
						json.recordsTotal = json.DATAS.VIDEO.TOTAL_ITEM; // Skip không quan tâm
						json.recordsFiltered = json.DATAS.VIDEO.TOTAL_ITEM; // Tính toán paging
					}
					json.draw = json.DATAS.VIDEO.DRAW; // Đồng bộ hóa giữa request - response của Datatables. Để biết chính xác đang xử lý response hành động nào trên table (Nhập nhanh filter, paging...)
					
					var arrData = [];
					// Map dữ liệu từ server về DataTables
					arrData = json.DATAS.VIDEO.DATA.map(function (item, index) {
						return {
							STT: index
							, FULL_DU_LIEU: item
							, ID: !isEmpty(item.ID) ? item.ID : ''
							, DANH_SACH_HINH_ANH_DAI_DIEN: item.DANH_SACH_HINH_ANH_DAI_DIEN
							, TIEU_DE_VIDEO: !isEmpty(item.TIEU_DE_VIDEO) ? item.TIEU_DE_VIDEO : ''
							, TIEU_DE_VIDEO_SLUG: !isEmpty(item.TIEU_DE_VIDEO_SLUG) ? item.TIEU_DE_VIDEO_SLUG : ''
							, TOM_TAT: !isEmpty(item.TOM_TAT_VIDEO) ? item.TOM_TAT_VIDEO : ''
							, VIDEO_NOI_BAT: item.VIDEO_NOI_BAT
		...
		, "columns": [
	    	  	{ 
					...
					, "render": function (data, type, row, meta) {
						let videoId = !isEmpty(row.ID) ? row.ID : '';
						let html = '<span data-id="' + videoId + '" >' + data + '</span>';
						return html;
					}
				} 
			  , { 
	          ...
	          , { 
	       	  ...
	       	  , { 
					"title": "Tiêu đề video"
					, "data": "TIEU_DE_VIDEO" // Dữ liệu lấy từ thuộc tính này
					...
					, "render": function (data, type, row, meta) {
						let pathUrl = '';
						if (!isEmpty(data)) {
							pathUrl = '{{ url("/admin/video/chi-tiet") }}' + '/' + row.TIEU_DE_VIDEO_SLUG + '-' + row.ID;
						}
			  ...
			  , { 
				...
				, { 
					"title": "Loại video"
					, "data": "VIDEO_NOI_BAT" // Dữ liệu lấy từ thuộc tính này
					...
					, "render": function (data, type, row, meta) {
						return !isEmpty(data) && data === true ? 'Video nổi bật' : 'Video thường';
					}
				}
	          , { 
					"title": "Hoạt động"
		...
		, "buttons": [
			// Button hiển thị danh sách theo CARD hoặc LIST
			{
				...
				, action: function () {
					// $('#tableVideo').DataTable().columns([0]).visible(false, true);
					// $('#tableVideo').DataTable().columns.adjust().draw(false); // Điều chỉnh kích thước cột và vẽ lại
				}
			}
		]
		...
		, "drawCallback": function(settings) {
			...
			const dropdownsOnTable = $("#tableVideo .dropdown-toggle");
			...
			const images = document.querySelectorAll("#tableVideo .td-img-thumnail > img");
			...
			const pageInfo = $('#tableVideo').DataTable().page.info();
			if (settings.iDraw == 2) { // Số lần vẽ bảng. 1 lần init empty table + 1 lần retrieve lần đầu tiên (First time)
				...
				$('#tableVideo').DataTable().page(startPageFrUrl).draw(false); // Draw bảng về trang nào
			}

			// Điều chỉnh lại kích thước cột
			$('#tableVideo').DataTable().columns.adjust();
        }
		...
		,"initComplete": function (settings, json) {
			...
			$("#tableVideo").wrap("<div style='overflow:auto; width: 100%; position:relative; max-height: 70vh;'></div>");
			console.log(this.api().rows().data());
		}
    });

	// Listen event Datatables 
	dataTableVideo.on('page', function (e, settings) { // Event thay đổi page
		// Get thông tin page info
		let pageInfo = dataTableVideo.page.info();
		...
		if (settings.iDraw !== 1) {
			settings.IS_CALL_AJAX = false;
			dataTableVideo.page('first').draw(false); // Chuyển về trang đầu và vẽ lại bảng
			...
			if (result.isConfirmed === true) {
				...
				$.ajax({
					...
					success: function(data, textStatus, request) {
    					...
    					dataTableVideo.row(row).remove().draw(false);
					}, 
    ...
    function switchActive(videoId, row) {
		...
		showSwalWarningPopup(function callback(result) {
			if (result.isConfirmed === true) {
				...
				$.ajax({
					type: "PUT", 
					url: '{{ url("/api/video/active") }}' + "/" + videoId, 
					contentType: "application/json",
					showLoading: true,
					data: JSON.stringify(data), 
					success: function(data, textStatus, request) {
    					...
    					let cellGiaTriActive = dataTableVideo.cell(row, '5');
    ...
    function fnSearchVideo() {
		...
		let length = window.wwAdminListUrl ? window.wwAdminListUrl.dataTableLength(10) : $('#tableVideo').DataTable().page.len();

		// Fetch api retrieve data
		dataTableVideo.page(0).page.len(length).ajax.reload(null, false);
    }
	...
	$.when(promiseLoadDataListStatusActive)
	.done(function(objTrangThaiHoatDong) {
		fnSearchVideo();
	})
	.fail(function(...args) {
