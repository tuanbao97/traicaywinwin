...
class NewsDetailRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        foreach ($permissions as $index => $permission) {
            if ($permission->CODE === PermissionEnum::QL_TIN_TUC_CHI_TIET->value) {
                return true;
            }
        }
        return false;
    ...
    public function rules(): array
    {
        $rules = [
            'ID' => [
                ...
                , 'required'
                , 'integer'
                , new CheckNotExistsFieldRule('news', 'ID', $this->route('ID'), 'USING')
            ]
        ];
    ...
    protected function failedValidation(\Illuminate\Contracts\Validation\Validator $validator)
    {
        ...
        if (!empty($idFailedRules)) {
            ...
            if ($idFailedRuleNotFound->isNotEmpty()) {
                throw new NotFoundException($validator->errors(), 'Tin tức không tồn tại.');
            }
        }
