...
class RegisterUserRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        // Check tồn tại email
        $rules['EMAIL'][] = new CheckDuplicateFieldRule('user', 'EMAIL', $this->EMAIL, 'USING');
    ...
    protected function failedValidation(\Illuminate\Contracts\Validation\Validator $validator)
    {
        ...
        if (!empty($validator->failed())) {
            foreach ($validator->failed() as $index => $validatorFailed) {
                if ('EMAIL' === $index) {
                    ...
                    $emailFailedRuleDuplicate = $collectionEmailFailedRules->filter(function($value, $key) {
                        return $key == CheckDuplicateFieldRule::class;
                    });
                    if ($emailFailedRuleDuplicate->isNotEmpty()) {
                        throw new NotFoundException($validator->errors(), 'Email đã tồn tại.');
                    }
                }
            }
        }
        ...
        throw new BadRequestException($validator->errors(), 'Đăng ký user thất bại.');
    }
}
