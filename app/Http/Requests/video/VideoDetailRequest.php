...
class VideoDetailRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        foreach ($permissions as $index => $permission) {
            if ($permission->CODE === PermissionEnum::QL_VIDEO_XEM->value) {
                return true;
            }
        }
        return false;
    ...
    protected function failedValidation(\Illuminate\Contracts\Validation\Validator $validator)
    {
        ...
        if (!empty($idFailedRules)) {
            ...
            if ($idFailedRuleNotFound->isNotEmpty()) {
                throw new NotFoundException($validator->errors(), 'Video không tồn tại.');
            }
        }
