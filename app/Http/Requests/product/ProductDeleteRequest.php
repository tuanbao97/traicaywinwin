...
class ProductDeleteRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        foreach ($permissions as $index => $permission) {
            if ($permission->CODE === PermissionEnum::QL_SAN_PHAM_XOA->value) {
                return true;
            }
        }
        return false;
    ...
    public function rules(): array
    {
        ...
        // Check tồn tại sản phẩm với status USING hoặc SOLD
        $rules['ID'][] = function($attribute, $value, $fail) {
            if (!DB::table('product')->where('ID', $value)->whereIn('STATUS', ['USING', 'SOLD'])->exists()) {
