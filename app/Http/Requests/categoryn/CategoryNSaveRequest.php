...
class CategoryNSaveRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        foreach ($permissions as $index => $permission) {
            if ($permission->CODE === PermissionEnum::QL_DANH_MUC_TIN_TUC_CHINH_SUA->value) {
                return true;
            }
        }
        return false;
    ...
    public function rules(): array
    {
        ...
        $rules['TEN_DANH_MUC_TIN_TUC'][] = new CheckDuplicateFieldRule('category_n', 'NAME', $this->TEN_DANH_MUC_TIN_TUC, 'USING', $id);
        if (!is_null($id)) { // Create
            $rules['ID'][] = new CheckNotExistsFieldRule('category_n', 'ID', $this->ID, 'USING');
        }

        // Check tồn tại parent_id
        $parent_id = $this->input('PARENT_ID');
        if (!is_null($parent_id)) {
            $rules['PARENT_ID'][] = new CheckNotExistsFieldRule('category_n', 'ID', $parent_id, 'USING');
        }
