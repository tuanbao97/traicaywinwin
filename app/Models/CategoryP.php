...
class CategoryP extends Model
{
    ...
    {
        return $this->belongsToMany(DocumentStorage::class, 'category_p_document_storage', 'CATEGORY_P_ID', 'DOCUMENT_STORAGE_ID')
            ...
            ->using(CategoryPDocumentStoragePivot::class)
            ...
            ->where(column: [
                ['category_p_document_storage.STATUS', '=', AppConstant::STATUS_USING]
                , ['category_p_document_storage.ATTR1', '=', 'DANH_SACH_HINH_ANH_DAI_DIEN']
            ])
            ->orderBy('category_p_document_storage.SORT_ORDER', 'ASC')
            ->orderBy('category_p_document_storage.ID', 'ASC');
    }
    ...
    public function childrens() {
        return $this->hasMany(CategoryP::class, 'PARENT_ID', 'ID')
        ->where(
           [
