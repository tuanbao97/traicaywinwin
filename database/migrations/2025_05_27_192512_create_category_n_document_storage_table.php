...
return new class extends Migration
{
    ...
    {
        Schema::create('category_n_document_storage', function (Blueprint $table) {
            $table->comment('Bảng danh mục tin tức document storage');
            $table->string('EXTENSION')->comment('Đuôi file');
            $table->bigInteger('ID', true)->comment('id');
            $table->bigInteger('CATEGORY_N_ID')->index('category_n_document_storage_fk')->comment('Danh mục sản phẩm id');
            $table->bigInteger('DOCUMENT_STORAGE_ID')->index('category_n_document_storage_fk_1')->comment('File id');
            $table->integer('SORT_ORDER')->nullable()->default(0)->comment('Thứ tự sắp xếp');
            $table->boolean('IS_THUMNAIL')->nullable()->default(false)->comment('Cờ có phải là thumnail chính của sản phẩm hay không');
    ...
    {
        Schema::dropIfExists('category_n_document_storage');
    }
};
