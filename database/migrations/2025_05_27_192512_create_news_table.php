...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('news', function (Blueprint $table) {
            $table->comment('Bảng tin tức');
            $table->bigInteger('ID', true)->comment('Tin tức id');
            $table->string('TITLE', 1000)->comment('Tiêu đề tin tức');
            $table->string('SUMMARY', 500)->comment('Tóm tắt tin tức');
            $table->longText('CONTENT_FORMAT')->comment('Nội dung tin tức format');
            $table->longText('CONTENT_RAW')->comment('Nội dung tin tức nguyên bản');
            $table->string('META_SEO_KEYWORDS', 1000)->nullable()->comment('Từ khóa SEO');
            $table->string('META_SEO_DESCRIPTION', 1000)->nullable()->comment('Mô tả SEO');
            $table->dateTime('APPROVED_DATE', 6)->nullable()->comment('Ngày approved tin tức');
            $table->dateTime('PUBLISHED_DATE', 6)->nullable()->comment('Ngày xuất bảng');
            $table->boolean('IS_HOT_NEWS')->nullable()->default(false)->comment('Đây có phải tin nổi bật hay không');
            $table->integer('COUNT_VIEWS')->nullable()->default(0)->comment('Số lượng lượt xem tin tức');
            $table->boolean('IS_APPROVED')->nullable()->comment('Tin tức đã được cấp trên approved hay chưa');
            $table->bigInteger('USER_POST_NEWS_ID')->index('news_fk')->comment('Người đăng bài');
            $table->bigInteger('USER_APPROVED_POST_NEWS_ID')->nullable()->index('news_fk_1')->comment('Người approved tin tức này');
