...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('user', function (Blueprint $table) {
            $table->comment('Bảng người dùng');
            $table->bigInteger('ID', true)->comment('Người dùng id');
            $table->string('EMAIL', 500)->comment('Email');
            $table->string('USERNAME', 500)->comment('User name');
            $table->string('PASSWORD', 2000)->nullable()->comment('Mật khẩu');
            $table->string('FULL_NAME', 500)->nullable()->comment('Họ và tên');
            $table->integer('COUNT_LOGIN_FAIL')->nullable()->default(0)->comment('Số lần đăng nhập thất bại');
            $table->string('ACTIVE_KEY', 2000)->nullable()->comment('Mã active tài khoản');
            $table->string('RESET_KEY', 2000)->nullable()->comment('Mã reset kích hoạt tài khoản');
            $table->dateTime('RESET_DATE', 6)->nullable()->comment('Ngày reset kích hoạt tài khoản');
