...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('transaction', function (Blueprint $table) {
            $table->comment('Bảng giao dịch');
            $table->bigInteger('ID', true)->comment('Giao dịch id');
            $table->bigInteger('USER_BUY_ID')->nullable()->index('transaction_fk')->comment('Người mua id');
            $table->string('USER_BUY_EMAIL', 1000)->nullable()->comment('Email của người mua');
            $table->string('USER_BUY_FULLNAME', 500)->nullable()->comment('Họ và tên người mua');
            $table->string('USER_BUY_PHONE')->nullable()->comment('Số điện thoại người mua');
            $table->longText('USER_BUY_ADDRESS')->nullable()->comment('Địa chỉ nhận hàng của người mua');
            $table->double('TOTAL_QUANTITY')->comment('Tổng số lượng sản phẩm mua');
            $table->double('TOTAL_PRICE')->comment('Tổng số tiền mua hàng');
            $table->longText('USER_BUY_MESSAGE')->nullable()->comment('Tin nhắn của người mua');
            $table->string('TRANSACTION_STATUS')->comment('Trạng thái của giao dịch');
            $table->string('PAYMENT_METHOD', 500)->nullable()->comment('Phương thức thanh toán');
            $table->dateTime('PAYMENT_DATE', 6)->nullable()->comment('Ngày thanh toán');
