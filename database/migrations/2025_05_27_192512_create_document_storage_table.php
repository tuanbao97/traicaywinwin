...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('document_storage', function (Blueprint $table) {
            $table->comment('Bảng file upload');
            $table->bigInteger('ID', true)->comment('Mã khóa chính');
            $table->string('NAME', 2000)->comment('Tên file đã mã hóa');
            $table->string('ORIGINAL_NAME', 2000)->nullable()->comment('Tên nguyên thủy của file');
            $table->string('EXTENSION')->comment('Đuôi file');
            $table->string('PATH', 2000)->comment('Đường dẫn file upload');
            $table->string('DIRECTORY', 2000)->comment('Thư mục chứa file upload');
            $table->double('SIZE')->nullable()->default(0)->comment('Kích thước file upload');
            $table->longText('MD5')->nullable()->comment('File mã hóa MD5');
            $table->string('TYPE_FILE')->comment('Loại file upload: image/pdf/attach file, ...');
