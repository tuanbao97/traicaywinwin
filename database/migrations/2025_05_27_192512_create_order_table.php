...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('order', function (Blueprint $table) {
            $table->comment('Bảng đặt hàng order');
            $table->bigInteger('ID', true)->comment('Order id');
            $table->bigInteger('TRANSACTION_ID')->index('order_fk')->comment('Giao dịch id');
            $table->bigInteger('PRODUCT_ID')->index('order_fk_1')->comment('Sản phẩm id');
            $table->double('QUANTITY')->comment('Số lượng sản phẩm');
            ...
            $table->double('PRICE')->comment('Giá cả');
