...
return new class extends Migration
{
    ...
    {
        Schema::create('video_document_storage', function (Blueprint $table) {
            $table->comment('Bảng video - file upload');
            $table->bigInteger('ID', true)->comment('id');
            $table->bigInteger('VIDEO_ID')->index('video_document_storage_fk')->comment('Video id');
            $table->bigInteger('DOCUMENT_STORAGE_ID')->index('video_document_storage_fk_1')->comment('File id');
            $table->integer('SORT_ORDER')->nullable()->default(0)->comment('Thứ tự sắp xếp');
            $table->boolean('IS_THUMNAIL')->nullable()->default(false)->comment('Cờ có phải là thumnail chính của video hay không');
            $table->string('TYPE')->comment('Loại file : image/ attact_file/ pdf/ ...');
            $table->string('EXTENSION')->comment('Đuôi file');
            ...
            $table->index(['VIDEO_ID', 'DOCUMENT_STORAGE_ID'], 'video_document_storage_idx');
            $table->index(['VIDEO_ID', 'STATUS'], 'video_document_storage_idx_1');
            $table->index(['DOCUMENT_STORAGE_ID', 'STATUS'], 'video_document_storage_idx_2');
            
            // Foreign key constraints
            $table->foreign('VIDEO_ID', 'video_document_storage_fk')
                  ->references('ID')
                  ->on('video')
                  ->onDelete('cascade')
                  ->onUpdate('cascade');
                  
            $table->foreign('DOCUMENT_STORAGE_ID', 'video_document_storage_fk_1')
                  ->references('ID')
                  ->on('document_storage')
                  ->onDelete('cascade')
                  ->onUpdate('cascade');
    ...
    {
        Schema::dropIfExists('video_document_storage');
    }
};
