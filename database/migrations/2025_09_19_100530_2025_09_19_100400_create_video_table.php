...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('video', function (Blueprint $table) {
            $table->comment('Bảng video');
            $table->bigInteger('ID', true)->comment('Mã khóa chính');
            $table->string('TITLE', 1000)->comment('Tên video');
            $table->string('SUMMARY', 500)->comment('Tóm tắt video');
            $table->text('CONTENT_FORMAT')->comment('Nội dung video format');
            $table->text('CONTENT_RAW')->comment('Nội dung video nguyên bản');
            ...
            $table->boolean('IS_HOT_VIDEO')->nullable()->default(false)->comment('Đây có phải video nổi bật hay không');
            $table->integer('COUNT_VIEWS')->nullable()->default(0)->comment('Số lượng lượt xem video');
