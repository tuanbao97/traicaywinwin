...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('product_variant', function (Blueprint $table) {
            ...
            $table->bigInteger('PRODUCT_ID')->nullable()->index('product_variant_product_fk');
            $table->string('PRODUCT_STATUS', 1000)->comment('Tình trạng sản phẩm');
            $table->string('PRODUCT_COLOR', 500)->comment('Màu sắc sản phẩm');
            $table->double('PRODUCT_STORAGE')->nullable()->comment('Dung lượng sản phẩm');
            $table->bigInteger('PRODUCT_IMAGE_ID')->nullable()->index('product_variant_document_storage_fk')->comment('Hình ảnh đại diện id');
            $table->boolean('IS_CONTACT_PRICE')->nullable()->default(false)->comment('Có phải giá liên hệ không');
            $table->double('PRODUCT_PRICE')->nullable()->comment('Giá bán');
            $table->double('PRODUCT_ORIGINAL_PRICE')->nullable()->comment('Giá gốc');
            $table->boolean('IS_IN_STOCK')->default(true)->comment('Còn hàng trong kho không');
