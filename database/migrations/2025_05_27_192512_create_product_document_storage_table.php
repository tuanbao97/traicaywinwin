...
return new class extends Migration
{
    ...
    {
        Schema::create('product_document_storage', function (Blueprint $table) {
            $table->comment('Bảng sản phẩm document storage');
            $table->bigInteger('ID', true)->comment('id');
            $table->bigInteger('PRODUCT_ID')->index('product_document_storage_fk')->comment('Sản phẩm id');
            $table->bigInteger('DOCUMENT_STORAGE_ID')->index('product_document_storage_fk_1')->comment('File id');
    ...
    {
        Schema::dropIfExists('product_document_storage');
    }
};
