...
return new class extends Migration
{
    ...
    {
        Schema::create('category_p_document_storage', function (Blueprint $table) {
            $table->comment('Bảng sản phẩm document storage');
            $table->string('EXTENSION')->comment('Đuôi file');
            $table->bigInteger('ID', true)->comment('id');
            $table->bigInteger('CATEGORY_P_ID')->index('category_p_document_storage_fk')->comment('Danh mục sản phẩm id');
            $table->bigInteger('DOCUMENT_STORAGE_ID')->index('category_p_document_storage_fk_1')->comment('File id');
    ...
    {
        Schema::dropIfExists('category_p_document_storage');
    }
};
