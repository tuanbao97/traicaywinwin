...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('user_profile', function (Blueprint $table) {
            $table->comment('Bảng profile của người dùng');
            $table->bigInteger('ID', true)->comment('Mã id');
            $table->bigInteger('USER_ID')->index('user_profile_fk')->comment('Mã người dùng');
            $table->string('MOBILE')->nullable()->comment('Số điện thoại');
            $table->longText('ADDRESS')->nullable()->comment('Địa chỉ');
            $table->boolean('IS_DEFAULT')->default(false)->comment('Cờ xác nhận đâu là profile mặc định');
            $table->longText('DESCRIPTION')->nullable()->comment('Mô tả');
            $table->bigInteger('AVATAR_ID')->nullable()->index('user_profile_document_storage_fk')->comment('Avatar id');
